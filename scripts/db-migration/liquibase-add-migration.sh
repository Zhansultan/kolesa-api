...
#There is no posix date command in MacOS, we will check if it has gdate,
# if no we will use default date for POSIX/Linux
gnudate() {
...
DEFAULT_VERSION="$(($(gnudate +%s%N)/1000000))"
NAME="$1"
CONTENT="$2"
RELATIVE_PATH="${3:-.}"
